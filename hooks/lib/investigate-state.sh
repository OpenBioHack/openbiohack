...
# Directory this lib lives in (for locating sibling libs like skill-token-lib.sh).
INVESTIGATE_LIB_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]:-$0}")" 2>/dev/null && pwd || echo "")"
...
investigate_is_active() {
    local cwd="$1" file="$2"
    # F4: hermetic test lane (and any explicit caller) can force activation without
    # living under a real run root. Production leaves this unset, so no behavior change.
    [ -n "${INVESTIGATE_SCOPE_OVERRIDE:-}" ] && return 0
...
investigate_record_read() {
    ...
    # F7: dedup on append — keeps the read-log bounded and makes the per-gate grep -Fxq
    # scans cheap. A read already logged this session is a no-op.
    if [ -f "$d/read-log" ] && grep -Fxq "$path" "$d/read-log" 2>/dev/null; then
        return 0
    fi
    printf '%s\n' "$path" >> "$d/read-log"
}
...
# F2: integrity-checked audit-token gate. Replaces the existence-only check on the
# write path. A token opens the gate ONLY if it is non-empty AND a valid HMAC AND bound
# to this exact gate name AND within the 2400s (40 min) audit-window TTL.
# Returns 0 (open) / non-zero (deny). FAIL-CLOSED: if verify_token cannot be loaded the
# gate denies rather than opening.
INVESTIGATE_AUDIT_TOKEN_TTL="${INVESTIGATE_AUDIT_TOKEN_TTL:-2400}"
investigate_verify_audit_token_anchored() {
    local session="$1" gate="$2" start="$3"
    local skill="investigate-audit-$gate"
    local sd td f tok
    sd="$(investigate_state_dir "$session")"
    f="$sd/audit-tokens/$gate.token"
    if [ ! -s "$f" ]; then
        td="$(investigate_run_token_dir "$start" 2>/dev/null)" || td=""
        [ -n "$td" ] && f="$td/$gate.token"
    fi
    [ -s "$f" ] || return 1
    tok="$(cat "$f" 2>/dev/null)"
    [ -n "$tok" ] || return 1
    if ! command -v verify_token >/dev/null 2>&1; then
        [ -n "${INVESTIGATE_LIB_DIR:-}" ] && [ -f "$INVESTIGATE_LIB_DIR/skill-token-lib.sh" ] \
            && . "$INVESTIGATE_LIB_DIR/skill-token-lib.sh"
    fi
    command -v verify_token >/dev/null 2>&1 || return 1   # fail-closed: cannot verify -> deny
    ( SKILL_TOKEN_TTL_SECONDS="$INVESTIGATE_AUDIT_TOKEN_TTL"; verify_token "$tok" "$skill" >/dev/null 2>&1 )
}
...
investigate_record_read_anchored() {
    ...
    [ -n "$d" ] || return 0
    # F7: dedup the run-root mirror too.
    if [ -f "$d/read-log" ] && grep -Fxq "$path" "$d/read-log" 2>/dev/null; then
        return 0
    fi
