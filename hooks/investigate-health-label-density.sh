...
# F6: newline-safe single-pass parse (replaces positional `sed -n '1p'/'2p'`,
# which a newline injected into file_path/session could shift to move attacker
# text into the CONTENT slot).
...
# --- portable scope guard: active iff cwd OR the file being written lives ---
# at/under a run root carrying the .investigate-active marker. No hardcoded
# project path.
