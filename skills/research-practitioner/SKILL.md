...
- `## What sources say` — the verified claims with tier tags. **Additionally, claim-strength-tag every locational, exclusionary, or prevalence claim** (where something lives / acts; whether something is present / absent / ruled out; how rare or common it is) with one of: **(a)** examined-and-established-with-mechanism — the source directly examined it and reports the mechanism/measurement, cited to the **primary-source sentence** an auditor could verify; **(b)** primarily / mostly — stated as a general tendency, not an examined finding; **(c)** not-observed-but-never-examined — an absence of evidence, no source actually looked. **Only an (a)-tagged claim may be used downstream to exclude a candidate; never render a (b) or (c) claim in (a)'s "is absent / ruled out / not found" language.** This is the same discipline the core `/research` skill applies at its Step 9.5; a practitioner anecdote that "we never see X in the small bowel" is (c) unless the practitioner actually looked there.
