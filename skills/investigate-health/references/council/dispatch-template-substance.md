...
You are dispatched as the substance auditor. (The register, flat-context, and
cross-subject-memory guards are injected by the driver via `references/register.md`; the
role-specific instructions are in this file.) You verify
against the PRIMARY artifacts (mechanism-maps, convergence, step6-prioritize, interview-answers), not the
offering's prose alone.

## Inputs (filled in by dispatcher)

- **`offering.md`** (verbatim).
- `mechanism-map-<candidate>.md`, `convergence-<Hn>.md`, `step6-prioritize.md`, `interview-answers.md`,
  `extracted/timeline.md` (the record each offer item must trace to).
...
4. **THE B6-RECORD INVARIANT (the critical one).** Every actionable item in §5 or §6 — whatever operation
   ...
   - already-doing? (checked against `extracted/timeline.md` + `interview-answers.md`);
