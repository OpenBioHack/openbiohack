...
ones: over-ranking (singling one possibility out as the cause), ranking-discipline meta-commentary,
flat hedging, process-completion-as-quality, a needless opt-in menu, a diagnosis in
...
You are dispatched as the register auditor. (The register, flat-context, and
cross-subject-memory guards are injected by the driver via `references/register.md`; the
role-specific instructions are in this file.) You read the
...
2. **Over-ranking / superlative** — "the single biggest", "the main driver", "the primary cause", "what's
...
3. **Flat all-equal hedge after the work is done** — refusing to lean when the elimination work has
   ...
   "everything is equally possible" is as much a FAIL as over-ranking one as the cause.
...
8. **Ranking-discipline meta-commentary (the leak this auditor most often misses)** — the offer must
   *do* the prioritisation, never *narrate* that it is prioritising. FAIL any literal "crown" /
   "crowning" / "not a crown" / "without crowning" / "held in parallel" / "we are not crowning a
   winner" / "no single culprit" used as a meta-aside about the ranking method, or any sentence whose
   job is to describe *how* the ranking is being done rather than to actually rank. Just rank; never
   comment on the act of ranking.
...
findings:
  - item: <the offending sentence, quoted>
    issue: <diagnosis-own-voice | over-ranking | ranking-meta-commentary | flat-hedge | process-completion-as-quality | needless-menu | advisory-tone | confidence-escalation | missing-D8-disclaimer | none>
