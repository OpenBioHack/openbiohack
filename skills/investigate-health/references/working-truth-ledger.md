...
(no-evidence ≠ negative); a population "usually a commensal" must not override a large-magnitude standout that
...
- **Gross physical/mechanistic implausibility** — e.g. a behavioural air-handling mechanism cannot produce
  gas continuously and in volume; the physics caps confidence regardless of how well other features fit.
- **Positive treatment-response that only one mechanism explains** — where an agent that acts only
  inside the gut lumen improves a symptom, that is a luminal fingerprint a behavioural cause cannot
  produce; it raises the luminal hypotheses and refutes behavioural.
